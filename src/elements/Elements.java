...
public enum Elements {
	...
	VAPOR(Color.LIGHT_GRAY, 275, 0.000169, 275, 2.257, 0.00001,/* WATER */
	null, 425, 0.00015, INF, 0, 0, null, 1.0, 0.2, 0.1, false),

	WATER(Color.BLUE, 275, 1.0, 277, 0.355, 0.0, ICE, 375, 0.918, 375, 2.257,
			0.0001, VAPOR, 0.9, 0.8, 0.6, false),
	...
	SALT_WATER(new Color(20, 225, 252), 254.0, 1.02, 256, .3557, 0.0001,
			SALT_ICE, 370.0, 1.03, 340, 2.257, 0.001, VAPOR, 0.9, 0.8, 0.6,
			false),
	...
	private Elements(Color color, double lowestTemp, double densityAtLowest,
		...
		this(color, lowestTemp, densityAtLowest, condensPoint, enthalpyCold,
				0.0, cooledState, highestTemp, densityAtHighest, vaporPoint,
				enthalpyHot, 0.0, heatedState, movabiltiy, heatExchangeSame,
				heatInertia, false);
	}

	private Elements(Color color, double lowestTemp, double densityAtLowest,
			double condensPoint, double enthalpyCold, double pCondens,
			Elements cooledState, double highestTemp, double densityAtHighest,
			double vaporPoint, double enthalpyHot, double pVapor,
			Elements heatedState, double movabiltiy, double heatExchangeSame,
			double heatInertia,

			boolean dynamic) {
		Element behaviour = new Element(this, color, lowestTemp,
				densityAtLowest, condensPoint, enthalpyCold, pCondens,
				cooledState == null ? null : cooledState.behaviour,
				highestTemp, densityAtHighest, vaporPoint, enthalpyHot, pVapor,
				heatedState == null ? null : heatedState.behaviour, movabiltiy,
				heatExchangeSame, heatInertia, dynamic);
